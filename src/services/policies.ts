...
export default class PoliciesService extends TransactionBaseService {
  ...
  async listAndCount(
    ...
    const policiesRepository = manager.withRepository(this.policiesRepository_)
    ...
    const query = buildQuery(selector, config) as FindManyOptions<Policies> & {
      where: {}
  ...
  async update(policyId: string, update: UpdatePolicy): Promise<Policies> {
    return await this.atomicPhase_(async (manager) => {
      const policiesRepository = manager.withRepository(
        this.policiesRepository_
      )
