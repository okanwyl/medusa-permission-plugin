...
import {buildQuery, ExtendedFindConfig, FindConfig, isString, Selector, TransactionBaseService,} from "@medusajs/medusa"
...
import {CreatePoliciesGroup, UpdatePoliciesGroup} from "../types/policies-group";
...
export default class PoliciesGroupService extends TransactionBaseService {
        ...
        return await this.atomicPhase_(async (manager) => {
            ...
            if (isDefined(policies)) {
                ...
                if (policies?.length) {
                    ...
                    policiesGroup.policies = policyIds.map(
                        (id) => ({id} as Policies)
                    );
                }
    ...
    async retrieve(policyGroupId: string, config: FindConfig<PoliciesGroup> = {}) {
