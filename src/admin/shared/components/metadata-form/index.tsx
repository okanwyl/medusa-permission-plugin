...
const MetadataForm = ({form}: MetadataProps) => {
    ...
    return (
        <>
            ...
            {ignoredLength > 0 && (
                <div
                    className="rounded-rounded p-base gap-x-base mt-base flex items-start border border-[#FFD386] bg-[#FFECBC]">
                    <div>
                        <ExclamationCircle
                            // fillType="solid"
                            // size={20}
...
const MetadataRow = ({
    ...
    return (
        <div className="last-of-type:rounded-b-rounded group/metadata relative">
            <div
                className="divide-grey-20 [&>div]:px-base [&>div]:py-xsmall grid grid-cols-[165px_1fr] divide-x divide-solid">
                {children}
            </div>
            <DropdownMenu.Root>
                ...
                <DropdownMenu.Content asChild sideOffset={8}>
                    <div
                        className="bg-grey-0 shadow-dropdown border-grey-20 rounded-rounded z-50 overflow-hidden border">
