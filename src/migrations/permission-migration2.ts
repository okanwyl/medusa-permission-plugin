...
export class MyMigration1617703530230 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.createForeignKey('policies_group_policies', new TableForeignKey({
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        const usersForeignKey = usersTable.foreignKeys.find(fk => fk.columnNames.indexOf('policies_group_id') !== -1);
        await queryRunner.dropForeignKey('user', usersForeignKey);

        await queryRunner.dropColumn('user', 'policies_group_id');

        const policiesGroupPoliciesTable = await queryRunner.getTable('policies_group_policies');
        const foreignKey1 = policiesGroupPoliciesTable.foreignKeys.find(fk => fk.columnNames.indexOf('policiesGroupId') !== -1);
        const foreignKey2 = policiesGroupPoliciesTable.foreignKeys.find(fk => fk.columnNames.indexOf('policyId') !== -1);

        await queryRunner.dropForeignKeys('policies_group_policies', [foreignKey1, foreignKey2]);
